...
function AutoCards(inHook, inText, inStop) {
    ...
    */
    ...
    function createLifeSimPossessionCard(event) {
        if (!event || !event.item || !event.recipient) {
            return false;
        }
        ...
        // Prevent duplicate possession cards.
    ...
    if (!HOOK) {
        ...
        case "input": {
            // AutoCards was called within the input modifier
            // LifeSim Auto-Cards V1: detect purchases and gifts from the player's original input.
            if (
                LIFESIM_ENABLED
    ...
    function getDefaultConfig() {
        ...
        return O.s({
            ...
            // Approximately how much shorter should recently compressed memories be? (ratio = 10 * old / new)
            memoryCompressionRatio: validateMemCompRatio(DEFAULT_MEM
ORY_COMPRESSION_RATIO),
            // Ignore all-caps during title candidate detection?
